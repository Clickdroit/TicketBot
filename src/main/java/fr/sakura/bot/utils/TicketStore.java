...
public class TicketStore {
	...
	public synchronized TicketEntry createTicket(String guildId, String userId, String channelId) {
		...
		try (Connection conn = DatabaseManager.getConnection();
			...
			pstmt.executeUpdate();
			return getOpenTicket(guildId, userId);
		} catch (SQLException e) {
	...
	public synchronized void claimTicket(String guildId, String channelId, String claimedBy) {
		String sql = "UPDATE tickets SET claimed_by = ? WHERE guild_id = ? AND channel_id = ? AND status = 'OPEN'";
		try (Connection conn = DatabaseManager.getConnection();
			 PreparedStatement pstmt = conn.prepareStatement(sql)) {
	...
	public synchronized void closeTicket(String guildId, String channelId, String closedBy, String closeReason) {
		String sql = "UPDATE tickets SET status = 'CLOSED', closed_by = ?, closed_at = ?, close_reason = ? WHERE guild_id = ? AND channel_id = ? AND status = 'OPEN'";
		try (Connection conn = DatabaseManager.getConnection();
			 PreparedStatement pstmt = conn.prepareStatement(sql)) {
	...
	public synchronized List<TicketEntry> getOpenTickets(String guildId) {
		...
		String sql = "SELECT * FROM tickets WHERE guild_id = ? AND status = 'OPEN' ORDER BY id DESC";
		try (Connection conn = DatabaseManager.getConnection();
			 PreparedStatement pstmt = conn.prepareStatement(sql)) {
			pstmt.setString(1, guildId);
			ResultSet rs = pstmt.executeQuery();
