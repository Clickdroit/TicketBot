...
public class MessageLogListener extends BaseLogListener {
    ...
    public MessageLogListener(String logChannelId) {
        super(logChannelId);
    }
    ...
    public void onMessageReceived(@NotNull MessageReceivedEvent event) {
        ...
        cacheMessageStatic(event.getMessage());
    }
    ...
    public void onMessageUpdate(@NotNull MessageUpdateEvent event) {
        ...
        CachedMessage cached = messageCache.get(event.getMessageId());
        ...
        findRecentAuditAction(event.getGuild(), ActionType.MESSAGE_DELETE, cached.authorId, AUDIT_TIMING_STANDARD, 3)
            .thenAccept(auditEntry -> {
                ...
                sendLogToChannel(event.getGuild(), embed -> {
                    ...
                    embed.setTitle(isGhostPing ? "👻  ✦  Ghost Ping Détecté" : (isModeratorAction ? "🔧  ✦  Message Supprimé (Staff)" : "🗑️  ✦  Message Supprimé"));
                    
                    StringBuilder desc = new StringBuilder();
                    desc.append(EmbedStyle.SEPARATOR).append("\n");
                    desc.append(isGhostPing ? "👻 **GHOST PING DÉTECTÉ**" : (isModeratorAction ? "🔧 **SUPPRESSION STAFF**" : "🗑️ **MESSAGE SUPPRIMÉ**")).append("\n");
                    desc.append(EmbedStyle.SEPARATOR).append("\n\n");
                    ...
                    if (!cached.attachments.isEmpty()) {
                        desc.append("\n📎 **Pièces jointes :**\n");
                        for (CachedAttachment att : cached.attachments) {
