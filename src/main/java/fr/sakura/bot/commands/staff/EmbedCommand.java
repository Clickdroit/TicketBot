...
/**
 * Commande permettant au staff d'envoyer des embeds personnalisés.
 */
public class EmbedCommand implements ICommand {
    ...
    public SlashCommandData getCommandData() {
        return Commands.slash(getName(), "Envoie un embed personnalisé dans un salon")
                .addOptions(
                        ...
                        new OptionData(OptionType.STRING,  "image",       "URL HTTPS d'une image (bannière)",        false),
    ...
    public void execute(SlashCommandInteractionEvent event) {
        ...
        // Résolution du salon cible avec gestion d'erreur de cast
        TextChannel channel;
        try {
        ...
        channel.sendMessageEmbeds(embed.build()).queue(
                ...
                err -> {
                    ...
                    event.reply("❌ Impossible d'envoyer l'embed dans ce salon (permissions manquantes ?).").setEphemeral(true).queue();
                }
        );
    }
